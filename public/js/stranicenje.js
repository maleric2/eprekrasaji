...
/* ZA POZIVANJE */
function setUPTableType2(tablica) {
    _current = {};
    _ukupno = {};
    _brojTablica = 0;
    $(tablica).find('tfoot').empty();
        //console.log($(this))
        _brojTablica++;
        _current[_brojTablica] = 0;
        _ukupno[_brojTablica] = CountRow(tablica);
        console.log(_ukupno[_brojTablica]);
        ShowTable(tablica, 0, 10);


        $(tablica).attr("val", _brojTablica);

        if ((_ukupno[_brojTablica] / 10) > 1) {
            $(tablica).append($("<tfoot/>").append($("<td colspan='4' class='table-buttons-btn'/>")
...
function setUPTable(tablica) {
    ...
    _brojTablica = 0;
        ...
        if ((_ukupno[_brojTablica] / 10) > 1) {
            $(tablica).append($("<tfoot/>").append($("<td colspan='4' class='text-center'/>")
                    .append("<button type='button' id='TablePageFirst' class='btn btn-success'><i class='glyphicon glyphicon-fast-backward'></i></button> "
                            + " <button type='button' id='TablePrevious' class='btn btn-primary'>Prethodni</button> "
                            + " <button type='button' id='TableNext' class='btn btn-primary'>Sljedeci</button> "
                            + " <button type='button' id='TablePageLast' class='btn btn-success'><i class='glyphicon glyphicon-fast-forward'></i></button> ")));
...
$(document).ready(function() {
    ...
    $("table").each(function() {
        ...
        if ((_ukupno[_brojTablica] / 10) > 1) {
            $(this).append($("<tfoot/>").append($("<td colspan='4' class='table-buttons-btn' />")
                    .append("<button type='button' id='TablePageFirst' class='btn btn-success'><i class='glyphicon glyphicon-fast-backward'></i></button> "
                            + " <button type='button' id='TablePrevious' class='btn btn-primary'>Prethodni</button> "
                            + " <button type='button' id='TableNext' class='btn btn-primary'>Sljedeci</button> "
                            + " <button type='button' id='TablePageLast' class='btn btn-success'><i class='glyphicon glyphicon-fast-forward'></i></button> ")));
